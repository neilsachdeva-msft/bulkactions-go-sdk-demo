...
const (
    ...
    defaultSubscriptionID = "4a627bba-c297-45a6-b9a4-459dbd2b738f"
...
func main() {
    ...
    if len(succeededVMs) > 0 {
        ...
        log.Printf("No succeeded VMs to delete for %s", baRegular1K)
    }

    // 4) Create 80K vCPUs of Spot priority using VM Attributes.
    ...
    // 7) Create 4 concurrent BAs of 20K each.
...
func setup() {
    ...
    bulkActionsClientFactory, err = armcomputebulkactions.NewClientFactory(subscriptionID, conn, nil)
    logIfError(err)
...
func createBulkActions(
    ...
    poller, err := bulkActionsClient.BeginCreateOrUpdate(ctx, resourceGroupName, location, bulkActionsName, parameters, nil)
    logIfError(err)
...
func listVMsInBulkAction(bulkActionsName string, operationStatus *armcomputebulkactions.VMOperationStatus) []*string {
    var allVMs []*string
    ...
    pager := bulkActionsClient.NewListVirtualMachinesPager(resourceGroupName, location, bulkActionsName, options)
    for pager.More() {
